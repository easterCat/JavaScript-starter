...
- [页面内容](#content)
- [减少 HTTP 请求数](#content-http)
- [减少 DNS 查询](#content-dns)
- [避免重定向](#content-redirect)
- [缓存 Ajax 请求](#content-ajax-cache)
- [延迟加载](#content-postload)
- [预先加载](#content-preload)
- [减少 DOM 元素数量](#content-dom)
- [划分内容到不同域名](#content-domains)
- [尽量减少 iframe 使用](#content-iframe)
- [避免 404 错误](#content-404)

- [服务器](#server)
- [使用 CDN](#server-cdn)
- [添加 Expires 或 Cache-Control 响应头](#server-cache)
- [启用 Gzip](#server-gzip)
- [配置 Etag](#server-etag)
- [尽早输出缓冲](#server-flush)
- [Ajax 请求使用 GET 方法](#server-ajax-get)
- [避免图片 src 为空](#server-src)

- [Cookie](#cookie)
- [减少 Cookie 大小](#cookie-size)
- [静态资源使用无 Cookie 域名](#cookie-free)

- [CSS](#css)
- [把样式表放在 `<head>` 中](#css-head)
- [不要使用 CSS 表达式](#css-expression)
- [使用 `<link>` 替代 `@import`](#css-link)
- [不要使用 filter](#css-filter)

- [JavaScript](#javascript)
- [把脚本放在页面底部](#javascript-bottom)
- [使用外部 JavaScript 和 CSS](#javascript-extenal)
- [压缩 JavaScript 和 CSS](#javascript-minify)
- [移除重复脚本](#javascript-duplicate)
- [减少 DOM 操作](#javascript-dom)
- [使用高效的事件处理](#javascript-event)

- [图片](#image)
- [优化图片](#image-optimize)
- [优化 CSS Sprite](#image-sprite)
- [不要在 HTML 中缩放图片](#image-scale)
- [使用体积小、可缓存的 favicon.ico](#image-favicon)

- [移动端](#mobile)
- [保持单个文件小于 25 KB](#mobile-25kb)
- [打包内容为分段（multipart）文档](#mobile-multipart)
